...
export const Navbar = ({ title, subtitle, onBack, children }: NavbarProps) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-zinc-900/80 backdrop-blur-xl p-4 md:px-6 md:py-4 rounded-2xl border border-white/5 shadow-lg shrink-0 w-full mb-6 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-64 h-64 bg-amber-500/5 rounded-full blur-3xl pointer-events-none -translate-x-1/2 -translate-y-1/2" />
      ...
      <div className="flex items-center gap-4 z-10 min-w-0">
        <motion.button 
          ...
          className="flex p-2 text-zinc-400 hover:text-amber-400 bg-zinc-800/50 hover:bg-zinc-800 rounded-xl transition-colors border border-white/5"
        ...
        {onBack && (
          <motion.button 
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onBack}
            className="flex items-center gap-2 text-zinc-400 hover:text-amber-400 font-bold transition-colors bg-zinc-800/50 hover:bg-zinc-800 px-3 py-2 rounded-xl border border-white/5"
          >
        ...
        <div className="flex flex-col min-w-0">
          {subtitle && <span className="text-[10px] md:text-xs font-bold text-amber-500 uppercase tracking-widest truncate block">{subtitle}</span>}
