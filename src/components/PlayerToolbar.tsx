...
export const PlayerToolbar = ({
  ...
  return (
    <div className={`bg-zinc-900/90 backdrop-blur-xl p-3 sm:p-4 md:p-5 rounded-2xl md:rounded-3xl shadow-2xl shadow-black/60 border flex flex-col md:flex-row gap-4 md:gap-6 items-center min-h-[60px] md:min-h-[80px] relative transition-colors ${isImmersiveMode ? 'border-amber-500/30' : 'border-white/10'}`}>
      ...
      {errorMsg ? (
        ...
        <div className="flex items-center gap-3 text-amber-500 w-full font-bold animate-pulse z-10 justify-center">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 w-full z-10">
          ...
          <div className="flex items-center gap-3 md:gap-6 w-full md:w-auto flex-1">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={togglePlay}
              className="flex items-center justify-center w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-amber-400 to-amber-600 text-zinc-950 rounded-full shadow-[0_0_20px_rgba(245,158,11,0.3)] shrink-0 disabled:opacity-50 border border-amber-300/50"
              disabled={isLoading}
            >
              {isPlaying ? (
            ...
            <div className="flex flex-col flex-1 max-w-[280px]">
              <span className="text-[10px] text-zinc-400 uppercase font-bold tracking-wider mb-1 flex items-center gap-1">
                <Guitar size={12} className="text-amber-500" /> Pista Visualizada
              </span>
                ...
                tuning.map((t, idx) => {
                  ...
                  return (
                    <div key={idx} title={tooltip} className="flex items-center bg-black/40 hover:bg-black/60 transition-colors rounded-md border border-white/5 overflow-hidden cursor-help">
                      ...
                      <span className="px-1.5 md:px-2 py-0.5 md:py-1 text-[10px] md:text-xs font-bold text-amber-500">
                        {t.note}
                      </span>
                    </div>
                  );
          ...
          <div className="hidden md:flex ml-4 pl-4 border-l border-white/5 items-center">
            <button
              ...
              className={`p-2 rounded-xl transition-all flex items-center justify-center shrink-0 ${isImmersiveMode ? 'bg-amber-500 text-zinc-950 shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'bg-zinc-900 border border-white/5 hover:bg-zinc-800 text-zinc-400 hover:text-white'}`}
