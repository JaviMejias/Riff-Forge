...
export const ChordSelector = ({ initialRoot, selectedChord, onSelectChord, mode = 'full' }: ChordSelectorProps) => {
  ...
  const boxWidth = mode === 'modal' ? 110 : 120;
  const boxHeight = mode === 'modal' ? 150 : 160;

  return (
    <div className="flex flex-col h-full w-full gap-4 p-8">
      ...
      <div className={`grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-4 overflow-y-auto custom-scrollbar p-2 ${mode === 'modal' ? 'max-h-[50vh]' : 'flex-1'}`}>
                ...
                <div className="relative">
                  ...
                  {chordDef.isCustom && chordDef.id && (
                    <button
                      ...
                      className="absolute -top-2 -right-2 p-2 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-full transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  )}
