...
import { Search, Volume2, Plus, Trash2, X } from 'lucide-react';
...
export const ChordSelector = ({ initialRoot, selectedChord, onSelectChord, mode = 'full' }: ChordSelectorProps) => {
  ...
  const boxWidth = mode === 'modal' ? 80 : 100;
  const boxHeight = mode === 'modal' ? 110 : 135;

  return (
    <div className={`flex flex-col h-full w-full min-h-0 gap-3 sm:gap-4 ${mode === 'modal' ? 'p-0 sm:p-2' : 'p-3 sm:p-6 lg:p-8'}`}>
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full min-h-11 bg-zinc-900/50 border border-white/10 rounded-xl sm:rounded-2xl py-2 sm:py-3 pl-10 pr-11 text-sm sm:text-base text-white placeholder-zinc-500 focus:outline-none focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-all shadow-inner"
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute inset-y-0 right-0 w-11 flex items-center justify-center text-zinc-500 hover:text-white" aria-label="Limpiar búsqueda">
              <X size={17} />
            </button>
          )}
        </div>
        <button
          ...
          className="min-h-11 flex items-center justify-center gap-2 px-4 sm:px-6 py-2 sm:py-3 bg-zinc-800 hover:bg-primary-500 hover:text-zinc-950 text-white font-bold text-sm sm:text-base rounded-xl sm:rounded-2xl transition-all border border-white/5"
        >
          <Plus size={18} />
          Crear Acorde
        </button>
      ...
      <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-2 -mx-2 px-2 shrink-0">
        <button
          ...
          className={`min-h-11 px-5 py-2 rounded-xl font-bold text-sm whitespace-nowrap transition-all ${activeRoot === 'ALL'
        ...
        {allRoots.map(root => (
          <button
            ...
            className={`min-h-11 px-5 py-2 rounded-xl font-bold text-sm whitespace-nowrap transition-all ${activeRoot === root
              ? 'bg-primary-500 text-zinc-950 shadow-[0_0_15px_rgba(245,158,11,0.3)]'
              : 'bg-zinc-800/80 text-zinc-400 hover:bg-zinc-700 hover:text-white border border-white/5'
              }`}
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-2 sm:gap-4 overflow-y-auto custom-scrollbar p-0.5 sm:p-2 flex-1 min-h-0">
        <AnimatePresence mode="popLayout">
            ...
            return (
              ...
              >
                <div className="relative">
                  ...
                  {chordDef.isCustom && chordDef.id && (
                    <button
                      ...
                      className="absolute -top-2 -right-2 w-10 h-10 sm:w-8 sm:h-8 flex items-center justify-center bg-zinc-950 sm:bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-full transition-colors border border-red-500/20"
                      aria-label={`Eliminar acorde ${chordDef.name}`}
                    >
                      <Trash2 size={14} />
                    </button>
                  )}
                ...
                <button
                  ...
                  className={`mt-auto min-h-11 flex items-center justify-center gap-2 w-full py-2 sm:py-3 rounded-xl transition-all font-bold text-xs sm:text-base ${isSelected
          ...
          {filteredChords.length === 0 && (
            ...
            >
              ...
              <p className="text-sm mt-1 text-center px-4">Prueba otra búsqueda o crea un acorde personalizado.</p>
              {searchQuery && <button onClick={() => setSearchQuery('')} className="mt-4 min-h-11 px-4 rounded-xl bg-zinc-800 text-white text-sm font-bold">Limpiar búsqueda</button>}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
